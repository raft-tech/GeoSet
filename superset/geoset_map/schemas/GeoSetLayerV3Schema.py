...
V3 promotes ``pointSize`` to a top-level key and makes it polymorphic: either a
...
class GlobalColoringSchemaV3(Schema):
    """Schema for global/default layer styling (V3).

    Identical to the V1/V2 ``GlobalColoringSchema`` except that ``pointSize``
...
class GeoSetLayerV3Schema(BaseGeoSetLayerSchema):
    """Schema for GeoSet map layer configuration (version 3).

    Changes from V2:
    - ``pointSize`` is promoted from ``globalColoring`` to a top-level key.
    - ``pointSize`` is polymorphic: either a plain positive number (static) or
      a ``PointSizeDynamicSchema`` object that scales point size by a data column.
    ...
    global_coloring = fields.Nested(
        GlobalColoringSchemaV3, required=True, data_key="globalColoring"
    )
    ...
    def upgrade_from_previous_version(data: dict[str, Any]) -> dict[str, Any]:
        ...
        Returns:
            The schema converted to V3 format.
        """
        upgraded = data.copy()
    ...
    @staticmethod
    def upgrade_from_v1(data: dict[str, Any]) -> dict[str, Any]:
        """Upgrade a V1 schema directly to V3 format (V1 → V2 → V3).

        Composes the V1→V2 and V2→V3 upgrade steps.
        ...
        Returns:
            The schema converted to V3 format.
        """
        v2_data = GeoSetLayerV2Schema.upgrade_from_previous_version(data)
        return GeoSetLayerV3Schema.upgrade_from_previous_version(v2_data)
