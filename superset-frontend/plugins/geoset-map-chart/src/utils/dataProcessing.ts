...
// --- Cache for parsed GeoJSON features per query payload ---
const rawFeaturesCache = new WeakMap<object, GeoJsonFeature[]>();
...
export function parseRawFeatures(
  rawData: DataRecord[],
  dimension?: string,
  filterNulls = true,
): GeoJsonFeature[] {
  ...
  // Check cache using first object as key (common in Superset)
  const cacheKey = rawData[0];
  if (rawFeaturesCache.has(cacheKey)) return rawFeaturesCache.get(cacheKey)!;

  const features = rawData
    .map((row: Record<string, any>) => {
      const geojsonRaw = row.geojson;

      try {
        const geometry =
          typeof geojsonRaw === 'string' ? JSON.parse(geojsonRaw) : geojsonRaw;
        if (!geometry && filterNulls) return null;

        const extraProps = dimension ? { [dimension]: row[dimension] } : {};
        ...
        console.warn('Error parsing geometry:', err);
        return null;
      }
    })
    .filter(
      (f): f is Exclude<typeof f, null> => f !== null,
    ) as GeoJsonFeature[]; // filter nulls if parsing failed or geometry is null

  rawFeaturesCache.set(cacheKey, features);
  return features;
}

export function normalizeNullCategory(
...
/**
 * Extracts the geometry type from a raw GeoJSON value.
 * Handles both string (JSON) and already-parsed object formats.
 */
export function getGeometryType(
  geojsonRaw: string | object | null | undefined,
): string | undefined {
  if (!geojsonRaw) return undefined;
