...
import { Row, Flyout } from './utilities/flyout';
...
export const DeckGLContainer = memo(
  forwardRef((props: DeckGLContainerProps, ref) => {
    ...
    const [flyoutRows, setFlyoutRows] = useState<Row[]>([]);
    const [flyoutOpen, openFlyout] = useState<boolean>(false);
    ...
    useImperativeHandle(
      ref,
      () => ({ setTooltip, setFlyoutRows, openFlyout }),
      [],
    );
    ...
    const getLayerObjects = useCallback(
      () => layerStates.map(layerState => layerState.layer) as Layer[],
      [layerStates],
    );
    ...
    return (
      <>
        ...
        <Tooltip tooltip={tooltip} display={{ width, height }} />
        {flyoutOpen && Flyout(flyoutRows, openFlyout)}
      </>
    );
  }),
);
...
export type DeckGLContainerHandle = typeof DeckGLContainer & {
  ...
  setFlyoutRows: (rows: Row[]) => void;
} & {
  openFlyout: (open: boolean) => void;
};
